...
fn test_eval() {
    ...
    for (input, vars, expected) in get_valid_test_cases() {
        ...
        assert!((result.unwrap() - expected).abs() < ERROR_MARGIN, "{}\nExpected: {}, got: {}", input, expected, result.unwrap());
    }
}
